...
import type { Plot, HarvestLog, SpendLog } from '../../../types/garden';
...
export default function StatsView({ plots, harvests, spends }: Props) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflowY: 'auto', WebkitOverflowScrolling: 'touch' as never, paddingBottom: 'calc(88px + var(--safe-b) + 16px)', animation: 'pgIn .25s ease' }}>
      ...
      <div style={{ background: 'var(--card)', borderRadius: 'var(--r-lg)', margin: '0 20px 14px', display: 'flex', boxShadow: 'var(--sh)', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, padding: '20px 16px', textAlign: 'center', borderLeft: '1px solid rgba(44,32,24,.08)' }}>
          ...
          <div style={{ fontSize: totS >= 1000 ? 32 : 44, fontWeight: 800, letterSpacing: -2, lineHeight: 1 }}>¥{totS.toFixed(0)}</div>
        </div>
      </div>
      ...
      {noData ? (
        ...
        <>
          {topV.length > 0 && (<>
            <SLabel>采摘排行</SLabel>
            <div style={chartCardStyle}>{topV.map(([v, c]) => <BarRow key={v} label={v} val={c} suffix="次" max={mV} color="var(--green)" />)}</div>
          </>)}
          {topC.length > 0 && (<>
            <SLabel>支出分类</SLabel>
            <div style={chartCardStyle}>{topC.map(([c, a]) => <BarRow key={c} label={c.split(' ')[1] ?? c} val={a} suffix="¥" max={mC} color="var(--red)" prefix="¥" />)}</div>
          </>)}
          {months.length > 0 && (<>
            <SLabel>月度对照</SLabel>
            <div style={tblCardStyle}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead><tr>{['月份','采摘次数','当月支出'].map(h => <th key={h} style={thStyle}>{h}</th>)}</tr></thead>
                <tbody>{months.map(m => (
                  <tr key={m}>
                    <td style={{ ...tdStyle, fontWeight: 700 }}>{m}</td>
                    <td style={{ ...tdStyle, color: 'var(--green)', fontWeight: 700 }}>+{mo[m].h}次</td>
                    <td style={{ ...tdStyle, color: mo[m].s ? 'var(--red)' : 'var(--t3)', fontWeight: mo[m].s ? 700 : 400 }}>{mo[m].s ? '¥' + mo[m].s.toFixed(2) : '—'}</td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
          ...
          {Object.keys(ps).length > 1 && (<>
            ...
            <div style={tblCardStyle}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead><tr>{['菜地','采摘','支出'].map(h => <th key={h} style={thStyle}>{h}</th>)}</tr></thead>
                <tbody>{Object.entries(ps).map(([pid, d]) => (
                  <tr key={pid}>
                    <td style={{ ...tdStyle, fontWeight: 700 }}>{pn(pid)}</td>
                    <td style={{ ...tdStyle, color: 'var(--green)', fontWeight: 700 }}>{d.h}次</td>
                    <td style={{ ...tdStyle, color: d.s ? 'var(--red)' : 'var(--t3)', fontWeight: d.s ? 700 : 400 }}>{d.s ? '¥' + d.s.toFixed(2) : '—'}</td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
...
function SLabel({ children }: { children: React.ReactNode }) {
  return <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--t3)', padding: '16px 22px 8px' }}>{children}</div>;
}

function BarRow({ label, val, suffix, max, color, prefix }: { label: string; val: number; suffix: string; max: number; color: string; prefix?: string }) {
  return (
...
const chartCardStyle: React.CSSProperties = { background: 'var(--card)', borderRadius: 'var(--r-lg)', margin: '0 16px 10px', padding: 20, boxShadow: 'var(--sh)' };
const tblCardStyle: React.CSSProperties   = { background: 'var(--card)', borderRadius: 'var(--r-lg)', margin: '0 16px 10px', overflow: 'hidden', boxShadow: 'var(--sh)' };
const thStyle: React.CSSProperties = { fontSize: 11, fontWeight: 700, color: 'var(--t3)', letterSpacing: '.08em', textTransform: 'uppercase', padding: '12px 16px', borderBottom: '1px solid rgba(44,32,24,.06)', textAlign: 'left' };
const tdStyle: React.CSSProperties = { fontSize: 14, color: 'var(--t1)', padding: '12px 16px', borderBottom: '1px solid rgba(44,32,24,.05)' };
