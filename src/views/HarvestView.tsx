...
import type { Plot, Marker, HarvestLog } from '../types';
import { saveHarvest, deleteHarvest, newId } from '../services/firebase';
...
export default function HarvestView({ plots, markers, harvests, curPlot }: Props) {
  ...
  useEffect(() => {
    const handler = () => {
      setHPlot(curPlot ?? '');
      setHVeg(''); setHAmt(''); setHNote(''); setHDate(today());
      setOpen(true);
    };
    ...
    <>
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 8, padding: '8px 16px 12px', overflowX: 'auto' }}>
          <select className="fsel" value={fp} onChange={e => setFp(e.target.value)} style={selStyle}>
      ...
      >
        ...
        <FormField label="蔬菜">
          <input style={inputStyle} value={hVeg} onChange={e => setHVeg(e.target.value)}
            placeholder="蔬菜名称" list="vdl-h" autoFocus />
...
function StatTile({ val, lbl, color }: { val: number; lbl: string; color: string }) {
  return (
    <div style={{ flexShrink: 0, background: 'var(--card)', borderRadius: 'var(--r-md)', padding: '16px 18px', minWidth: 110, boxShadow: 'var(--sh)', scrollSnapAlign: 'start' }}>
