...
import type { Plot, SpendLog, SpendCategory } from '../types';
import { saveSpend, deleteSpend, newId } from '../services/firebase';
...
export default function SpendView({ plots, spends, curPlot }: Props) {
  ...
  const handleSave = async () => {
    ...
    await saveSpend({
      id: newId(), plotId: sPlot || null,
      cat: sCat, item: sItem.trim(), amt,
      date: sDate, note: sNote.trim(),
      createdAt: Date.now(),
    });
    setOpen(false);
  };
  ...
  return (
    <>
      <div style={{
        ...
        {filtered.length > 0 && (
          <div style={{
            background: 'var(--red-l)', borderRadius: 'var(--r-md)',
            margin: '0 16px 12px', padding: '14px 18px',
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--red)' }}>💰 合计支出</span>
            <span style={{ fontSize: 24, fontWeight: 800, color: 'var(--red)', letterSpacing: '-.8px' }}>
              ¥{total.toFixed(2)}
            </span>
          </div>
        )}
            ...
            <div key={s.id} style={{ ...lcStyle, animation: 'slideUp .22s ease' }}>
              ...
              <div style={{ flex: 1, padding: '14px', minWidth: 0 }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--t2)', marginTop: 3 }}>
                  {pn(s.plotId)}{pn(s.plotId) && s.note ? ' · ' : ''}{s.note}
                </div>
              </div>
      ...
      >
        ...
        <FormField label="项目名称">
          <input style={inputStyle} value={sItem} onChange={e => setSItem(e.target.value)}
            placeholder="番茄种苗 × 10株" autoFocus />
        </FormField>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <FormField label="金额（元）">
            <input style={inputStyle} type="number" value={sAmt} onChange={e => setSAmt(e.target.value)}
